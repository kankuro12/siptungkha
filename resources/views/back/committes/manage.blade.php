@extends('back.app')
@section('title','Committee manage')
@section('content')
<div class="header"  style="background:#890E4F; color:#ffffff; height:55px; margin:0px;">
    <center><p style="padding:20px; font-size:18px; font-family:"> Committee Manage</h3></p>
</div>

<section class="container">
 <div class="row" style=" padding-top:2rem;" >
        <div class="cell-md-12" >
            <form method="post"  action="/admin/committe/member/add/"  id="memberadd">
                    <div class="row">
                        <div class="cell-4">
                            <input type="hidden" name="committe_id"  value="{{$committe->id}}">
...
<hr>
 <div class="container">
         <table class="table table-border cell-border" id="members" >
            ...
            <tbody>
                @foreach($committe->committemember as $cm)
                    <tr id="member-{{$cm->id}}">
                        <td>{{$cm->id}}</td>
                        <td>{{$cm->member->name}}</td>
                        <td id="memberdeg-{{$cm->id}}">{{$cm->designation}}</td>
                        <td><button  onclick="openModel({{$cm->id}});" class="button link" >Edit</button>||<button class="button link" onclick="del({{$cm->id}});" >Delete</button></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
...
@section('script')
    <script>
        function update(){
            ...
            axios.post('/admin/committe/member/edit/',formData)
        ...
        function addMember(){
            ...
            axios.post('/admin/committe/member/add/',formData)
             .then(function(response){
                data=response.data;
                console.log(data);
                var member=data.data.member;
        ...
        function del(id){
        axios.post('/admin/committe/member/del/',{
            'id':id,
        })
            .then(function(response){
